...
def add_products():
    for i in range(5):
        Name = input ("ingresa el nombre del producto: ").strip ()
...
def calculateTotalValue():
    ...
    while True:
                answer=input("deseas añadir un nuevo producto?(si/no/salir)")
                if answer.lower()=="salir":
                    break
                elif answer.lower ()== "si":
                    add_products()
                else:
                    print("introduce 'si', 'no' o 'salir'.")

#main menu
def menu():
    while True:
        print("\n--- Menú para gestión de inventario ---")
        print("1. añade los 5 productos iniciales")
